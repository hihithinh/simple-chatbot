...
const fetchItems = async () => {
  ...
  try {
    ...
    const response = await ResponseService.getResponses(params)
...
const deleteItem = async () => {
  if (selectedItem.value) {
    try {
      await ResponseService.deleteResponse(selectedItem.value.id)
      await fetchItems()
...
<template>
  <CardBoxModal v-model="isModalDangerActive" title="Xác nhận xóa" button="danger" has-cancel @confirm="deleteItem">
    <p>Bạn có chắc chắn muốn xóa response này không?</p>
